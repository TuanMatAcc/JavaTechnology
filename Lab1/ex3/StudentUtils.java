...
public class StudentUtils {
    ...
    {
        double avgClass = list.stream().mapToDouble(Student :: average).sum();
        return avgClass/list.size();
    }
    ...
    {
        List<String> names = list.stream().filter(Student::isGoodStudent).map(st -> st.name).toList();
        return names;
    } 
}
